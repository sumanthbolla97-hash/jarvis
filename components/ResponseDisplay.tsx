  ...
  const renderContent = (text: string) => {
    ...
    return sections.map((section, sectionIdx) => {
      if (section.startsWith('```')) {
        ...
        return (
          <div key={sectionIdx} className="my-4 relative group">
      ...
      return paragraphs.map((para, paraIdx) => {
        ...
        if (para.includes('\n* ') || para.includes('\n- ') || para.startsWith('* ') || para.startsWith('- ')) {
          ...
          return (
            <ul key={`${sectionIdx}-${paraIdx}`} className="space-y-1.5 mb-4 ml-5 list-disc text-neutral-800 text-lg sm:text-2xl leading-relaxed">
        ...
        return (
          <p key={`${sectionIdx}-${paraIdx}`} className="mb-4 last:mb-0 text-neutral-900 text-lg sm:text-3xl font-medium leading-[1.4] tracking-tight antialiased">
            {renderSpans(para)}
          </p>
        );
      });
  ...
  return (
    <div className="max-w-5xl animate-in fade-in slide-in-from-bottom-8 duration-1000">
      ...
      <style>{`
        ...
        h2 { font-size: 1.5rem; letter-spacing: -0.02em; border-bottom: 2px solid #f5f5f5; padding-bottom: 0.25rem; margin-bottom: 0.5rem; }
